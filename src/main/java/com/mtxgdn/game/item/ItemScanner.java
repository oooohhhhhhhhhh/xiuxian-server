...
public class ItemScanner {
    ...
    private static final String ITEM_PACKAGE = "data.mtxgdn.item";
    ...
    public static ScanResult scanAndRegister() {
        ...
        List<Class<?>> classes;
        try {
            classes = findClassesInPackage(ITEM_PACKAGE);
        } catch (Exception e) {
        ...
        for (Class<?> clazz : classes) {
            try {
                if (!Item.class.isAssignableFrom(clazz)) {
                    skipped.add(clazz.getSimpleName() + " (未继承Item)");
                    continue;
                }
    ...
    public static class ScanResult {
        ...
        public String toString() {
            ...
            sb.append("物品扫描结果: 注册 ").append(registered.size())
