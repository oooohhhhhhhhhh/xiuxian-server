...
public class SecretRealmScanner {

    private static final String REALM_PACKAGE = "data.mtxgdn.secretrealm";

    private SecretRealmScanner() {
    }
    ...
    public static ScanResult scanAndRegister() {
        ...
        List<Class<?>> classes;
        try {
            classes = findClassesInPackage(REALM_PACKAGE);
        } catch (Exception e) {
        ...
        for (Class<?> clazz : classes) {
            try {
                if (!SecretRealm.class.isAssignableFrom(clazz)) {
                    skipped.add(clazz.getSimpleName() + " (未继承SecretRealm)");
                    continue;
                }
                if (clazz.equals(SecretRealm.class)) {
    ...
    public static class ScanResult {
        ...
        public String toString() {
            ...
            sb.append("[SecretRealmScanner] 扫描完成: ")
